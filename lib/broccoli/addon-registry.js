...
class AddonRegistry {
  ...
  get components() {
    ...
    this._components = addons.reduce((components, addon) => {
      let pathToAddon = path.join('node_modules', addon.name, 'app/components/g-map-addons');

      if (addon.project.addonPackages[addon.name]) {
        pathToAddon = path.join(addon.project.addonPackages[addon.name].path, 'app/components/g-map-addons');
      }
