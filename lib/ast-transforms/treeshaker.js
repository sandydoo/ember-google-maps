...
function makeTreeshaker({ included, excluded, isProduction }) {
  return class Treeshaker {
    constructor(env) {
      this.moduleName = env.moduleName;
      this.syntax = env.syntax;

      this.shouldExcludeName = newExcludeName(included, excluded);
      this.shouldSkipTreeshaking = skipTreeshaking(included, excluded);
    }

    transform(ast) {
      if (this.shouldSkipTreeshaking) {
        return ast;
      }
      ...
      let shouldExcludeName = this.shouldExcludeName;
